...
def run(rank, n_gpus, hps):
  ...
  if rank == 0:
    ...
    eval_loader = DataLoader(eval_dataset,shuffle=True,
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
  ...
  for batch_idx, items in enumerate(train_loader):
    ...
    assert hps.model.use_spk is not False, ("The model uses the speaker embeddings to conduct VC, thus \"use_spk\" should be set to True")
    c, spec, y, spk = items
    ...
    with autocast(enabled=hps.train.fp16_run):
      y_hat, ids_slice,_  = net_g(c, spec, g=g, mel=mel)
