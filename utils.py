...
def load_wav_to_torch_normalized(full_path):
...
def load_filepaths_and_text(filename, split=","):
  with open(filename, encoding='utf-8') as f: 
    ...
    #filepaths_and_text=[line.strip().split(split) for line in f if os.path.exists(line.strip().split(split)[0])]
...
def get_hparams(init=True):
  ...
  parser.add_argument('-c', '--config', type=str, default="./configs/exvc.json",
                      help='JSON file for configuration')
  parser.add_argument('-m', '--model', type=str,default="freevc",
                      help='Model name')
