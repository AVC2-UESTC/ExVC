...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def get_audio(self, filename):
        ...
        # get the ssl features (suing the artificial datasets ssl features)
        c_filename = source_filename.replace("DUMMY_ART","wavlm").replace(".wav", ".pt")
        c = torch.load(c_filename).squeeze(0)
        
        # get the target file. it should be the real one from original VCTK
        target_filename=filename.replace("_ART","").split("__")[0]+".wav"     
        audio, sampling_rate = load_wav_to_torch(target_filename)
        ...
        spec_filename = target_filename.replace(".wav", ".spec.pt")
        ...
        # get the spk embeddings. 
        spk_filename = target_filename.replace("DUMMY","spk").replace(".wav",".npy")
